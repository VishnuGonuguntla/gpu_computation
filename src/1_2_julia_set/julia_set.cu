...
//         data[idx+2] = 0;
//     } else {
//         // smooth sine palette — no speckles, no harsh jumps
//         float t = (float)iter / maxIter;
//         float scale = (iter < 5) ? 0.0f : 1.0f;   // clamp far outside to black
//         data[idx+0] = (unsigned char)(scale * (128 + 127 * sinf(t * 6.2831f + 0.0f)));
//         data[idx+1] = (unsigned char)(scale * (128 + 127 * sinf(t * 6.2831f + 2.094f)));
//         data[idx+2] = (unsigned char)(scale * (128 + 127 * sinf(t * 6.2831f + 4.189f)));
//     }
// }
...
int main() {
    ...
    stbi_write_png("julia1.png", width, height, 3, data, width * 3);
    printStats(end - start, width*height, maxIter, 0, 0);
    cudaFree(data);
    return 0;
}
