...
__global__ void kernelInitSolver(double *d_pos,
                      double *d_vel,
                      double *d_acc, 
                      double *d_mass,
                      int nParticles,int gridSize, double mass,
                      double *raw, double spacing) {
    int i = blockDim.x * blockIdx.x + threadIdx.x;
    int j = blockDim.y * blockIdx.y + threadIdx.y;
    int k = blockDim.z * blockIdx.z + threadIdx.z;
    
    int n = i * gridSize * gridSize + j * gridSize + k;
    if (i < gridSize && j < gridSize && k < gridSize && n < nParticles) {
        d_mass[n] = mass;
...
__global__ void kernelComputeForceLJ(double *d_pos, 
    ...
    if (i < nParticles) {
        for (int j = 0; j < nParticles; j++) {
            if (i==j) continue;
            double x = d_pos[3*i + 0] - d_pos[3*j + 0]; 
            double y = d_pos[3*i + 1] - d_pos[3*j + 1]; 
            double z = d_pos[3*i + 2] - d_pos[3*j + 2];
            ...
            double sr12 = sr6 * sr6;                
            double constval = 24.0 * eps * (2 * sr12 - sr6) / dist2;
...
__global__ void kernelFirstIntegratePBC(double *d_pos,
                                        double *d_vel,
                                        double *d_acc,
    ...
    if (i < nParticles) {
        d_pos[3*i + 0] = d_pos[3*i + 0] + d_vel[3*i + 0] * timeStep + 0.5 * d_acc[3*i + 0] * timeStep * timeStep;
        d_pos[3*i + 1] = d_pos[3*i + 1] + d_vel[3*i + 1] * timeStep + 0.5 * d_acc[3*i + 1] * timeStep * timeStep;
        d_pos[3*i + 2] = d_pos[3*i + 2] + d_vel[3*i + 2] * timeStep + 0.5 * d_acc[3*i + 2] * timeStep * timeStep;
...
__global__ void kernelFinalIntegratePBC(double *d_vel,
                                        double *d_acc,
                                        int nParticles, double timeStep) {
    int i = blockDim.x * blockIdx.x + threadIdx.x;
    ...
    if (i < nParticles) {
        ...
        for (int j = 0; j < i; j++ ) {
            ...
            if (dist2 > 1e-10) {
                ...
                atomicAdd(pEnergy, 4 * eps * (sr12 - sr6));

            }
            // }
        }
        atomicAdd(totalEnergy, KE + LDEnergy); //    totalEnergy += KE + 0.5 * LDEnergy;
    }
